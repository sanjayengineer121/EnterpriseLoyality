...
options = ["Fixed Date Coupons", "Variable Date Coupons", "Birthday Coupons", "Anniversary Coupons"]
combo_var = tk.StringVar()
coupon_combo = ttk.Combobox(root, textvariable=combo_var, values=options, state="readonly",width=100)
coupon_combo.set("Select an option")
coupon_combo.pack(pady=10,padx=0)

# Bind the event handler to the Combobox
coupon_combo.bind("<<ComboboxSelected>>", on_option_selected)

font = ('arial', 14, 'bold')

lb=tb.Label(bootstyle="info",text="Coupon heading",font=font)
lb.place(x=100,y=100)

e = tb.Entry()
e.configure(state="success")
e.place(x=100,y=130,width=400)

font = ('arial', 14)

lb1=tb.Label(bootstyle="primary",text="Coupon title one",font=font)
lb1.place(x=100,y=170)

font = ('arial', 14)

lb1=tb.Label(bootstyle="primary",text="Coupon title two",font=font)
lb1.place(x=500,y=170)


e1 = tb.Entry()
e1.configure(state="success")
e1.place(x=100,y=210,width=250)

e2 = tb.Entry()
e2.configure(state="success")
e2.place(x=500,y=210,width=250)

font = ('arial', 14)

lb1=tb.Label(bootstyle="success",text="Start Date",font=font)
lb1.place(x=100,y=250)

e3 = tb.Entry()
e3.configure(state="success")
e3.place(x=100,y=290,width=400)

lb1=tb.Label(bootstyle="success",text="Expiry Date",font=font)
lb1.place(x=100,y=330)

e4 = tb.Entry()
e4.configure(state="success")
e4.place(x=100,y=370,width=400)

lb1=tb.Label(bootstyle="success",text="Remark",font=font)
lb1.place(x=100,y=410)

e5 = tb.Entry()
e5.configure(state="success")
e5.place(x=100,y=450,width=400)
